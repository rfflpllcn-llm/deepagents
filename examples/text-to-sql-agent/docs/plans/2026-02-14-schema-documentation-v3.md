# Database Schema Documentation Implementation Plan (v3)
...
**Tech Stack:** DeepAgents, LangChain, PostgreSQL, langchain_community.tools, pytest
...
**Files:**
- Modify: `AGENTS.md:7-12,89,97`

**Step 1: Update "Your Role" section to 6-step workflow**
...
With:
```markdown
- Reference schema → Write query → Validate → Execute COUNT query with WHERE page = 42
```

**Step 4: Verify changes**

```bash
grep -n "schema-reference\|sql_db_query_checker" AGENTS.md
```

Expected: Should show the updated lines with skill and tool references.

**Step 5: Verify AGENTS.md size unchanged**

```bash
wc -l AGENTS.md
```

Expected: ~105 lines (same as before, minimal changes only)

**Step 6: Commit the AGENTS.md updates**
...
Add validation step with sql_db_query_checker.
...
Reduced from 117 lines to ~65 lines.
...
**Step 3: Comment out unused import (line 9)**
...
def test_only_query_and_checker_tools():
    """Verify agent has only sql_db_query and sql_db_query_checker tools."""
    ...
    assert tool_names == {"sql_db_query", "sql_db_query_checker"}, \
        f"Expected {{sql_db_query, sql_db_query_checker}}, got {tool_names}"
...
def test_agents_md_size_unchanged():
    """Verify AGENTS.md size is approximately unchanged (~105 lines)."""
    agents_path = Path("AGENTS.md")
    line_count = len(agents_path.read_text().splitlines())

    # Allow ±5 lines variance
    assert 100 <= line_count <= 110, \
        f"AGENTS.md should be ~105 lines (minimal changes), got {line_count} lines"
...
```bash
head -n 10 tests/test_agent_configuration.py
```

Expected: Should show docstring and imports.
...
Expected: Tests should FAIL because changes haven't been implemented yet.

**Step 5: Commit the test file**

```bash
git add tests/test_agent_configuration.py
git commit -m "test: add automated tests for agent configuration

Verify:
- Only query and checker tools present (no discovery tools)
- schema-reference skill exists
- schema-exploration skill deleted
- query-writing skill updated
- AGENTS.md updated and size unchanged
...
## Task 7: Verify Tool Availability

**Files:**
- Modify (temporarily): `agent.py:55`

**Step 1: Add debug print to verify tools**

After the `sql_tools` creation (around line 55), add:
```python
...
```bash
uv run pytest tests/test_agent_configuration.py::test_only_query_and_checker_tools -v
```

Expected: Test should PASS

**Step 5: Commit removal of debug code**
...
## Task 10: Test Complex JSONB Query
...
## Task 13: Test Error Handling - Unauthorized Query
...
## Task 14: Test Error Handling - Invalid Query
...
## Task 15: Run Full Automated Test Suite

**Files:**
- Test: `tests/test_agent_configuration.py`
...
- ✅ test_only_query_and_checker_tools
- ✅ test_schema_reference_skill_exists
- ✅ test_schema_exploration_deleted
- ✅ test_query_writing_skill_exists
- ✅ test_agents_md_updated
- ✅ test_agents_md_size_unchanged

**Step 2: Verify test coverage**

```bash
uv run pytest tests/test_agent_configuration.py -v --tb=short
```

Expected: Clean pass with no failures or warnings.

**Step 3: Document test results**

If all tests pass, proceed to final verification. If any fail, debug and fix before continuing.
...
7. Various test verification commits

**Step 2: Verify success criteria**

Check all criteria from design document:
- [ ] schema-reference skill contains comprehensive schema documentation
- [ ] AGENTS.md updated to remove discovery tool references (~105 lines, no growth)
- [ ] Agent has only `sql_db_query` and `sql_db_query_checker` tools
- [ ] schema-exploration skill is removed
- [ ] query-writing skill is simplified (~65 lines) and references schema-reference skill
- [ ] All manual test queries execute successfully
...
- [ ] Smaller baseline prompt (schema loaded on-demand)
- [ ] Clear error messages for invalid queries
- [ ] Correct imports used (QuerySQLDatabaseTool not deprecated version)

**Step 3: Measure baseline prompt size**
...
Expected: ~105 lines (minimal change from original ~105 lines)
...
All tests passing (6/6 automated + manual queries).
Agent uses schema-reference skill (progressive disclosure).
Query validation working. Baseline prompt size unchanged (~105 lines).
...
- [ ] schema-reference skill created with detailed tables, indexes, triggers, views
- [ ] AGENTS.md updated (lines 8-9, 89, 97), stays lean (~105 lines)
- [ ] query-writing skill simplified (~65 lines), references schema-reference, no SELECT *, all have LIMIT
- [ ] schema-exploration skill deleted
- [ ] agent.py uses QuerySQLDatabaseTool and QuerySQLCheckerTool (correct imports)
- [ ] Automated tests created (6 tests in test_agent_configuration.py)
- [ ] All automated tests pass
- [ ] Simple query test passes
- [ ] Join query test passes
- [ ] JSONB query test passes
- [ ] Full-text search test passes
- [ ] View query test passes
- [ ] Write operation rejected (safety)
- [ ] Query checker catches syntax errors
- [ ] Invalid table query handled gracefully
- [ ] Agent invokes schema-reference skill when needed (progressive disclosure working)
- [ ] Smaller baseline prompt confirmed (schema not in AGENTS.md, ~105 lines unchanged)
...
- Automated tests provide regression protection for tool selection behavior
